...
bool MultipleViewTriangulation::triangulation_all(
    const Matrix34d Prj[],
    int CamNumAll,
    MatrixXd x[],
    Vector3d Xr[],
...
void MultipleViewTriangulation::getRMS2(
    Matrix34d Prj[],
    int CamNumAll,
    MatrixXd xy[],
    Vector3d Xr[],
    ...
    double rms[])
{
    ...
    for (int i = 0; i < ptNum; i++)
    {
        ...
        xyc[i].resize(2, CamNumAll);
    }

    
    
    optimal_correction_all(Prj,
                           CamNumAll,
                           xy,
                           xyc,
                           idx,
...
void MultipleViewTriangulation::getRMS3(
    Matrix34d Prj[],
    int CamNumAll,
    MatrixXd xy[],
    Vector3d Xr[],
    ...
    double rms[])
{
    ...
    for (int i = 0; i < ptNum; i++)
    {
        ...
        for (int cnum = 0; cnum < CamNumAll; cnum++)
        {
            ...
            //std::cout << xy[i](cnum,0) << " " << xy[i](cnum,1) << " " << pxy(0,0) << " " << pxy(1,0) << "\r\n";
            sum = sqrt(pow(pxy(0, 0) - xy[i](0, cnum), 2) + pow(pxy(1, 0) - xy[i](1, cnum), 2));
            if(sum > rms[i])
            {
                rms[i] = sum;
            }
...
cv::Mat MultipleViewTriangulation::Get3DR_TransMatrix(const std::vector<cv::Point3f> &srcPoints, const std::vector<cv::Point3f> &dstPoints)
{
    ...
    cv::Mat R_T = (cv::Mat_<double>(4, 4) << matR.at<double>(0, 0), matR.at<double>(0, 1), matR.at<double>(0, 2), delta_X,
